...
export default function AdminPage() {
  ...
  // Ambil data berita langsung dari data publik github pas masuk halaman
  const fetchBerita = async () => {
    try {
      ...
      console.error("Gagal meload daftar berita");
    }
  };
  ...
  // 🔥 Fungsi Batal Sakti Pembersih Form
  const handleBatal = () => {
    setFormOpen(false);
    setIsEdit(false);
    setOldFilename('');
  ...
  const handleSimpanBerita = async (e) => {
    ...
    // Bikin nama file URL otomatis clean tanpa spasi
    ...
    try {
      const res = await fetch('/api/admin/berita', {
        ...
        body: JSON.stringify({ username, password, filename, title, thumb, body, isEdit })
      });
      const hasil = await res.json();

      if (res.ok) {
        showPesan('sukses', hasil.message);
        setFormOpen(false);
        fetchBerita();
      } else {
        showPesan('error', hasil.error);
      }
    } catch (err) {
      showPesan('error', 'Koneksi ke server terputus!');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleHapusBerita = async (filename, judul) => {
    if (!confirm(`Yakin mau hapus berita: "${judul}"? Tindakan ini permanen bre!`)) return;
    setLoading(true);
    try {
      const res = await fetch('/api/admin/berita', {
        method: 'DELETE',
      ...
      showPesan('error', 'Gagal menghapus data!');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{ padding: '20px', fontFamily: 'sans-serif', background: 'linear-gradient(135deg, #e0f2fe 0%, #f3e8ff 100%)', minHeight: '100vh', color: '#1e293b' }}>
      ...
      {!isLoggedIn ? (
        <div style={{ maxWidth: '400px', margin: '10vh auto', background: 'rgba(255,255,255,0.7)', backdropFilter: 'blur(10px)', padding: '30px', borderRadius: '24px', boxShadow: '0 20px 40px rgba(0,0,0,0.05)', textAlign: 'center', border: '1px solid rgba(255,255,255,0.5)' }}>
          ...
          <p style={{ color: '#64748b', fontSize: '0.9rem', marginBottom: '20px' }}>Silakan masuk menggunakan kredensial Vercel Anda.</p>
        ...
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
            ...
            <div style={{ background: 'white', padding: '25px', borderRadius: '24px', boxShadow: '0 20px 40px rgba(0,0,0,0.05)', marginBottom: '30px' }}>
              ...
              <form onSubmit={handleSimpanBerita} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                  ...
                  <input type="text" placeholder="Contoh: Siswa SMPN 1 Damai Juara Coding Tingkat Provinsi" value={title} onChange={(e) => setTitle(e.target.value)} style={{ padding: '12px', borderRadius: '12px', border: '1px solid #cbd5e1', fontSize: '1rem' }} required />
                </div>

                <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                  <label style={{ fontWeight: 'bold', fontSize: '0.9rem' }}>URL Foto Sampul (Thumbnail)</label>
                  <input type="text" placeholder="Kosongkan jika ingin memakai logo sekolah bawaan resmi" value={thumb} onChange={(e) => setThumb(e.target.value)} style={{ padding: '12px', borderRadius: '12px', border: '1px solid #cbd5e1', fontSize: '1rem' }} />
                </div>

                <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                  <label style={{ fontWeight: 'bold', fontSize: '0.9rem' }}>Isi Konten Berita</label>
                  <textarea placeholder="Tuliskan berita lengkap di sini. Gunakan enter untuk paragraf baru..." rows="10" value={body} onChange={(e) => setBody(e.target.value)} style={{ padding: '12px', borderRadius: '12px', border: '1px solid #cbd5e1', fontSize: '1rem', resize: 'vertical' }} required></textarea>
                </div>
          ...
          {/* LIST BERITA YANG SUDAH TERBIT */}
          {!formOpen && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              <h3 style={{ margin: '10px 0 5px 0' }}>Berita yang sudah Mengudara ({beritaList.length})</h3>
              {beritaList.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '40px', background: 'rgba(255,255,255,0.3)', borderRadius: '20px' }}>Sedang mengambil gudang berita dari GitHub...</div>
              ) : (
                beritaList.map((b) => (
